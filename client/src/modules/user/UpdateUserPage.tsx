    ...
    <form onSubmit={onSubmit}>
        <div>
          ...
          <Select id="userTypes"  required {...register('type')} >
            {userTypesQuery.isFetched && userTypesQuery?.data?.ok && 
              userTypesQuery.data.data!.map(
        ...
        <div>
          ...
          <Select id="roles" multiple required onChange={onChangeRole}>
            {rolesQuery.isFetched && rolesQuery.data?.ok && 
              Object.values(rolesQuery.data.data as object).map(
                x => <option value={x} key={x} >{x}</option>
              )
            }
          </Select>
